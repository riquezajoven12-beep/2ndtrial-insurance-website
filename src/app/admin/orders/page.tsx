...
async function getData() {
  ...
  return { enquiries: enquiries || [], isSA, agents };
}
...
export default async function OrdersPage() {
  ...
  return (<div>
    <div className="mb-8"><h1 className="font-serif text-2xl text-deep mb-1">{data.isSA ? 'All Enquiries' : 'My Enquiries'}</h1>
    <p className="text-sm text-gray-400">Product quote requests ({data.enquiries.length})</p></div>
    <EnquiriesTable enquiries={data.enquiries} isSA={data.isSA} agents={data.agents} />
  </div>);
}
